...
async function installConditions() {
  ...
  const url = `modules/{MODULE_ID}/data/conditions.json`.replace("{MODULE_ID}", MODULE_ID);
  ...
  const list = sorted.map(c => ({ id:c.id, name:c.name, label:c.name, img:`/modules/${MODULE_ID}/${c.icon}`, icon:`/modules/${MODULE_ID}/${c.icon}` }));
...
Hooks.once("init", ()=>{
  registerSettings();
  const link = document.createElement("link"); link.rel="stylesheet"; link.href=`modules/${MODULE_ID}/styles/styles.css`; document.head.appendChild(link);
});
  ...
  Hooks.once("init", () => {
    try {
      if (typeof buildSortedMarvelStatuses === "function") {
        ...
        const icon = `modules/marvel-multiverse-conditions-hud/assets/m.png`;
